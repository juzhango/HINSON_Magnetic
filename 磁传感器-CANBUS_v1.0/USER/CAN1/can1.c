...
/**
  * @brief  Fpclk1的时钟在初始化的时候设置为42M,
  *         如果设置CAN_Normal_Init(CAN_SJW_1tq,CAN_BS2_6tq,CAN_BS1_7tq,6,CAN_Mode_LoopBack);
...
{		   		   
 	u32 i;
	CanRxMsg RxMessage;
	if( CAN_MessagePending(CAN1,CAN_FIFO0)==0){printf("no msgs!");return 0;}		//没有接收到数据,直接退出 
